   ...
   <page-panel title="房态数据管理">
    ...
    <section slot="page-panel-body">
      ...
      <div class="_search-header">
        <el-form :inline="true" :model="queryObj">
          ...
          <el-form-item>
            ...
            <el-button icon="el-icon-arrow-up" type="primary" @click="onLine(1)" :loading="isOnLineLoading">批量上线</el-button>
            <el-button icon="el-icon-arrow-down" type="danger" @click="onLine(0)" :loading="isOffLineLoading">批量下线</el-button>
          </el-form-item>
        </el-form>
      </div>
      <!-- 列表 -->
      <el-table :data="tableData" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" prop="supplierId" label="供应商" width="240"> 
        ...
        <el-table-column align="center" label="上线状态">
          <template slot-scope="scope">
            <el-tag :type="scope.row.used == false ? 'gray' : 'success'">{{ scope.row.used == false ? "已下线": "已上线"}}</el-tag>
          </template>
        </el-table-column>
...
export default {
  ...
  methods:{
    async onLine(status){
      let txt=status===1?'上线':'下线'
      if(this.multipleSelection.length==0){
        this.$message.info( `请选择要${txt}的产品`)
        return
      }
      let filterData=filter(this.multipleSelection,item=>item.used)
      if(status===1){
        this.isOnLineLoading=true
        filterData=filter(this.multipleSelection,item=>!item.used)
      }else{
        this.isOffLineLoading=true
      }
      let params={status:status,data:JSON.stringify(filterData)}
      try {
        ...
        // 同意
        let result = await this.$api.company.productOnOffline(params)
